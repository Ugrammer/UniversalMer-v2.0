...
# OVERVIEW
<p>Universal-Mer is the k-mer counting tool for all possible size of k at once. More than typical k-mer counting tool, the program can summarize the counting result of  1-mers to  l-mers exactly at once where l = the length of longest repeated substring in the input sequence which now is set the maximum length to 100000-mers. The program can  report exactly the number of all repeat (freq>1) and unique(freq = 1) mers without cutting off any low frequency mers. For all k-mers where k>l, they surely occur only once in the input so it is easy to compute the number of them by L-k+1. </p>  
