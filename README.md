...
<p>The option of canonical form counting can be chosen in the program menu. The canonical k-mer form counting may require a quite more time than the ordinary form. The time and memory for building database depend on the length of input sequence (m) and  the longest repeated substring in the sequence (n), which are in O(mn).In the experiment of our work, the program used 30 GB of memory and 3000 - 4000 seconds for building the database from the input sequence 200 million bp(Fragaria vesca). </p>
